...
if __name__ == "__main__":
    ...
    createFolder('./full_e3_%d' % args.seed)
    for epoch in range(1, args.epochs + 1):
        ...
        if test_loss < best_loss:
            best_loss = test_loss
            best_model = copy.deepcopy(model.state_dict())
            save_checkpoint(epoch, model, optimizer, './full_e3_%d/%d.pth.tar' % (args.seed, epoch))
        ...
        epochs.append(epoch)
    with open("./full_e3_%d/loss.txt" % args.seed, "w") as f:
        f.write("Train Loss\n")
    ...
    plt.figure()
    plt.plot(epochs, trainlosslist, 'o', markersize=1, label='Train')
    plt.plot(epochs, testlosslist, 'o', markersize=1, label='Test')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.grid(True)
    plt.savefig('./full_e3_%d/loss.jpg' % args.seed)
