...
class ConvLayer(nn.Module):
    ...
    def forward(self, atom_in_fea, nbr_fea, atom_nbr_fea):
        ...
        nbr_sumed = nbr_filter * nbr_core #여기서 곱은 ew 곱. shape는 N, M, afl
...
class CrystalGraphConvNet(nn.Module):
    ...
    def __init__(self, orig_atom_fea_len, orig_nbr_fea_len, number_of_bonds, 
        ...
        self.embedding_nbr = nn.Linear(orig_atom_fea_len, atom_fea_len)
        self.embedding_nbr2 = nn.Linear(atom_fea_len, orig_atom_fea_len)
        self.pembedding_atom = nn.Linear(number_of_bonds, 1)
        self.pembedding_atom2 = nn.Linear(1, number_of_bonds)
        self.pembedding_bond = nn.Linear(number_of_bonds, 1)
        self.pembedding_bond2 = nn.Linear(1, number_of_bonds)
        self.bn = nn.BatchNorm1d(nbr_fea_len)
        self.bn2 = nn.BatchNorm1d(atom_fea_len)
        self.embedding_mu = nn.Linear(2*atom_fea_len+nbr_fea_len, l_dim)
        self.embedding_logvar = nn.Linear(2*atom_fea_len+nbr_fea_len, l_dim)
        self.decode = nn.Linear(l_dim, 2*atom_fea_len+nbr_fea_len)
    ...
    def forward(self, atom_fea, nbr_fea, nbr_fea_idx, crystal_atom_idx):
        ...
        atom_nbr_fea = self.embedding_nbr(atom_nbr_fea)
        _, L1 = atom_fea.shape
        _, _, L2 = nbr_fea.shape
        for conv_func in self.convs:
            atom_fea, nbr_fea, atom_nbr_fea = conv_func(atom_fea, nbr_fea, atom_nbr_fea)
        non_pooled_fea = torch.cat([atom_fea, atom_nbr_fea.view(N1, -1), nbr_fea.view(N1, -1)], dim=1)
        ...
        atom_nbr_fea = self.pooling(atom_nbr_fea, crystal_atom_idx)
        atom_nbr_fea = self.conv_to_fc_softplus(atom_nbr_fea)
        pooled_fea = torch.cat([atom_fea, atom_nbr_fea.view(N2, -1), nbr_fea.view(N2, -1)], dim=1)
        nbr_fea = nbr_fea.view(N2, L2, -1)
        nbr_fea = self.pembedding_bond(nbr_fea)
        nbr_fea = self.bn(nbr_fea.squeeze())
        atom_nbr_fea = atom_nbr_fea.view(N2, L1, -1)
        atom_nbr_fea = self.pembedding_atom(atom_nbr_fea)
        atom_nbr_fea = self.bn2(atom_nbr_fea.squeeze())
        crys_fea = torch.cat([atom_fea, atom_nbr_fea, nbr_fea], dim=1)
        ...
        z_decoded, z_decoded_nbr, decoded_nbr = torch.split(z_decoded, [L1, L1, L2], dim=1)
        decoded_nbr = self.pembedding_bond2(decoded_nbr.unsqueeze(2))
        decoded_nbr = self.bn(decoded_nbr.view(-1, L2))
        decoded_nbr = decoded_nbr.view(N2, -1)
        z_decoded_nbr = self.pembedding_atom2(z_decoded_nbr.unsqueeze(2))
        z_decoded_nbr = self.bn2(z_decoded_nbr.view(-1, L1))
        z_decoded_nbr = z_decoded_nbr.view(N2, -1)
        z_decoded = torch.cat([z_decoded, z_decoded_nbr, decoded_nbr], dim=1)
        z_decoded = self.fc_to_conv_softplus(self.decoder(z_decoded, non_pooled_fea, pooled_fea, crystal_atom_idx))
        z_decoded, z_decoded_nbr, decoded_nbr = torch.split(z_decoded, [L1, M1*L1, M1*L2], dim=1)
        decoded_nbr = decoded_nbr.view(N1, -1, L2)
        z_decoded_nbr = z_decoded_nbr.view(N1, -1, L1)
        ...
        z_decoded_nbr = self.embedding_nbr2(z_decoded_nbr)
